...
class Knife {
    ...
    async findById (id, type) {
        ...
        let item = _.omit(block.items[itemId], ['info']);
        item.isPoly = type === 'node' ? false : tagsUtils.isPolygonFeature(item.tags);
        return item;
    }
    ...
    relationToGeoJson (relation) {
        ...
        return turfPolygonize(collection);
    ...
    wayToGeoJson (way) {
        ...
        if(way.isPoly) {
            return turfHelpers.polygon(points).geometry;
        } else {
    ...
    getS2CellsGeo(geo) {
        ...
        const s2LLs = geo.coordinates[0].map(([lng, lat]) => (new s2.LatLng(lat, lng)));

        s2LLs.pop();
        const covering = s2.RegionCoverer.getCoveringTokens(s2LLs, config);

        console.log('cells: ', covering.length);
        console.log(covering.join(','));
